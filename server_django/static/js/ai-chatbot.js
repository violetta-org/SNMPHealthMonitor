/**
 * ═══════════════════════════════════════════════════════════════════════════
 * AI DevOps Assistant — Frontend Chat Widget
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * Features:
 * - Floating chat toggle with pulse animation
 ...
 * - Send button lock during request (anti-spam)
 * - Graceful error handling with toast notifications
 * - Auto-scroll to latest message
 * - Enter to send, Shift+Enter for new line
 * - Auto-detect sysname from page context
 */
...
(function () {
    ...
    const AI_HEALTH_URL = '/api/ai/health-summary';
    ...
    // ── Auto-detect sysname from page ───────────────────────────────────
    function getSysname() {
        // Try from body data attribute (dashboard pages)
        const body = document.body;
        if (body.dataset.sysname) return body.dataset.sysname;

        // Try from URL path: /dashboard/{sysname}/...
    ...
    function createChatWidget() {
        ...
        toggleBtn.title = 'AI DevOps Assistant';
        toggleBtn.innerHTML = `
        ...
        // Bind events
        bindEvents(toggleBtn, chatWindow);

        // Show welcome message
        addBotMessage(
            '👋 Xin chào **Quản trị viên**! Tôi là **AIOps Assistant** — trợ lý AI chuyên phân tích sức khỏe hệ thống.\n\n' +
            'Bạn có thể hỏi tôi về tình trạng CPU, RAM, Disk, Network, hoặc bấm các nút gợi ý bên dưới để bắt đầu nhanh.'
        );
    }

    // ── Event Bindings ──────────────────────────────────────────────────
    function bindEvents(toggleBtn, chatWindow) {
        const input = document.getElementById('ai-chat-input');
        const sendBtn = document.getElementById('ai-send-btn');
        const clearBtn = document.getElementById('ai-btn-clear');
        const analyzeBtn = document.getElementById('ai-btn-analyze');
        ...
        // Auto-analyze (health summary)
        analyzeBtn.addEventListener('click', () => {
            const sysname = getSysname();
            if (!sysname) {
                addBotMessage('⚠️ Không xác định được thiết bị. Vui lòng truy cập trang Dashboard của một thiết bị cụ thể để sử dụng tính năng phân tích tự động.');
                return;
            }
            addUserMessage('🔍 Phân tích sức khỏe tổng quan');
            fetchHealthSummary(sysname);
        });
    ...
    function sendMessage() {
        if (isLoading) return; // Anti-spam: block while loading

        const input = document.getElementById('ai-chat-input');
        const message = input.value.trim();

        // ── Validation ──────────────────────────────────────────────────
        if (!message) return; // Empty → ignore silently

        if (message.length > MAX_MESSAGE_LENGTH) {
            showToast(`Tin nhắn quá dài (tối đa ${MAX_MESSAGE_LENGTH} ký tự).`, 'error');
            return;
        }
        ...
        // ── Call AI API ─────────────────────────────────────────────────
        const sysname = getSysname();
        fetchAIResponse(message, sysname);
    }

    // ── Fetch AI Response ───────────────────────────────────────────────
    async function fetchAIResponse(message, sysname) {
        setLoading(true);
        showTypingIndicator();
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 422) {
                    ...
                    let errMsg = 'Dữ liệu không hợp lệ.';
                    if (Array.isArray(detail) && detail.length > 0) {
                        errMsg = detail[0]?.msg || errMsg;
                    }
                    addErrorMessage(errMsg);
                } else {
                    addErrorMessage(`Lỗi server (HTTP ${response.status}). Vui lòng thử lại.`);
                }
                return;
            }
            ...
            if (data.reply) {
                ...
                addErrorMessage('AI không trả về phản hồi. Vui lòng thử lại.');
            }
            ...
            if (err.name === 'TypeError' && err.message.includes('fetch')) {
                addErrorMessage('Không thể kết nối đến server. Kiểm tra lại kết nối mạng.');
            } else {
                addErrorMessage('Đã xảy ra lỗi không mong muốn. Vui lòng thử lại.');
            }
        } finally {
            setLoading(false);
        }
    ...
    async function fetchHealthSummary(sysname) {
        setLoading(true);
        showTypingIndicator();

        try {
            const response = await fetch(`${AI_HEALTH_URL}/${sysname}`, {
                method: 'GET',
            ...
            if (!response.ok) {
                addErrorMessage(`Lỗi khi phân tích (HTTP ${response.status}).`);
                return;
            }
            ...
            if (data.ok && data.summary) {
                ...
                addErrorMessage('Không thể phân tích lúc này.');
            }

        } catch (err) {
            removeTypingIndicator();
            addErrorMessage('Lỗi kết nối khi phân tích sức khỏe.');
        } finally {
            setLoading(false);
        }
    }
    ...
    function addErrorMessage(text) {
        const messages = document.getElementById('ai-chat-messages');
        const msgEl = document.createElement('div');
        msgEl.className = 'ai-message bot error';
        msgEl.innerHTML = `
