...
function createChatWidget(){const toggleBtn=document.createElement('button');toggleBtn.className='ai-chat-toggle';toggleBtn.id='ai-chat-toggle';toggleBtn.title='AI DevOps Assistant';toggleBtn.innerHTML=`
            ...
            </svg>`;const chatWindow=document.createElement('div');chatWindow.className='ai-chat-window';chatWindow.id='ai-chat-window';chatWindow.style.display='none';chatWindow.innerHTML=`
            <!-- Header -->
...
function bindEvents(toggleBtn,chatWindow){const input=document.getElementById('ai-chat-input');const sendBtn=document.getElementById('ai-send-btn');const clearBtn=document.getElementById('ai-btn-clear');const analyzeBtn=document.getElementById('ai-btn-analyze');const quickActions=document.getElementById('ai-quick-actions');toggleBtn.addEventListener('click',()=>{isOpen=!isOpen;if(isOpen){chatWindow.style.display='flex';chatWindow.offsetHeight;}
chatWindow.classList.toggle('open',isOpen);toggleBtn.classList.toggle('active',isOpen);if(isOpen){toggleBtn.style.animation='none';input.focus();}else{toggleBtn.style.animation='';setTimeout(()=>{if(!isOpen)chatWindow.style.display='none';},300);}});sendBtn.addEventListener('click',()=>sendMessage());input.addEventListener('keydown',(e)=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendMessage();}});input.addEventListener('input',()=>{input.style.height='auto';input.style.height=Math.min(input.scrollHeight,80)+'px';});clearBtn.addEventListener('click',()=>{const messages=document.getElementById('ai-chat-messages');messages.innerHTML='';chatHistory=[];addBotMessage('🔄 Lịch sử chat đã được xóa. Bạn có thể bắt đầu cuộc trò chuyện mới.');});analyzeBtn.addEventListener('click',()=>{const sysname=getSysname();if(!sysname){addBotMessage('⚠️ Không xác định được thiết bị. Vui lòng truy cập trang Dashboard của một thiết bị cụ thể để sử dụng tính năng phân tích tự động.');return;}
...
function scrollToBottom(){if(!isOpen)return;const messages=document.getElementById('ai-chat-messages');if(messages){requestAnimationFrame(()=>{const last=messages.lastElementChild;if(last)last.scrollIntoView({behavior:'smooth',block:'end'});});}}
